...
Le protocole corrigé a été appliqué avec un RandomForest (`class_weight=balanced`)
et une `GroupKFold` à 5 plis regroupée par fichier. Trois configurations ont été
comparées sur SUCRE, puis le modèle structurel a été évalué sur Sylius.
...
(AUC-ROC 1,000 par fuite de cible) à un modèle honnête et défendable
(AUC-ROC 0,853, MCC 0,66 sur SUCRE), obtenu à partir des seules caractéristiques
structurelles du code et évalué sans fuite de groupe. La performance se dégradant
fortement sur un projet à configuration YAML, le domaine d'applicabilité du
prédicteur est explicitement borné aux codebases Symfony annotés dans le code,
résultat cohérent avec la difficulté connue de la prédiction de défauts
inter-projets (Zimmermann et al. 2009).
